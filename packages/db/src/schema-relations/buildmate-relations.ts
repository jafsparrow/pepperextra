...
import { user, session, account, organization, team, member, invitation, teamMember } from "../auth-schema"
...
import { productTags, productTagAssignments } from "../schemas/tags"
...
export const buildmateRelations = defineRelations(
  {
    organization,
    team,
    user,
  ...
  (r) => ({
    ...
    products: {
      ...
      qrCodes: r.many.qrCodes(),
      purchaseReceipts: r.many.purchaseReceipts(),
      warrantyClaims: r.many.supplierWarrantyClaims(),
    },
    productLocationOverrides: {
    ...
    productTags: {
      ...
      team: r.one.team({
        from: r.productTags.teamId,
        to: r.team.id,
      }),
      assignments: r.many.productTagAssignments(),
    },
    productTagAssignments: {
    ...
    fulfillmentStations: {
      ...
      team: r.one.team({
        from: r.fulfillmentStations.teamId,
        to: r.team.id,
      }),
    ...
    invoices: {
      ...
      team: r.one.team({
        from: r.invoices.teamId,
        to: r.team.id,
      }),
      issuedByUser: r.one.user({
        from: r.invoices.issuedBy,
        to: r.user.id,
      }),
    ...
    purchaseReceipts: {
      ...
      team: r.one.team({
        from: r.purchaseReceipts.teamId,
        to: r.team.id,
      }),
      supplier: r.one.suppliers({
    ...
    qrCodes: {
      ...
      scannedByUser: r.one.user({
        from: r.qrCodes.scannedBy,
        to: r.user.id,
      }),
    },
  })
