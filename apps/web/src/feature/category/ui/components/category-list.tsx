...
import { CATEGORY_QUERY_KEYS, STOCK_TRACKING_LABELS } from "../../constants"
...
export function CategoryList({ orgId }: CategoryListProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return (categories ?? []).filter(
      (c) =>
        !q ||
        c.specName.toLowerCase().includes(q) ||
        c.brandPriority?.some((b) => b.toLowerCase().includes(q))
    )
          ...
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-muted-foreground/30 bg-muted/20 py-10 text-center">
            ...
            <p className="mt-1 max-w-xs text-xs text-muted-foreground">
              {searchQuery
                ...
                : "Add your first category to organize your product catalog."}
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-xs text-muted-foreground">
                      {category.brandPriority && category.brandPriority.length > 0
                        ? category.brandPriority.join(" · ")
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="hidden text-xs font-semibold sm:inline-flex">
                    {STOCK_TRACKING_LABELS[category.stockTrackingMode]}
                  </Badge>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40">
                      <CategoryModal orgId={orgId ?? ""} category={category}>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This will remove the category from your catalog. This action cannot
              be undone.
            </DialogDescription>
          </DialogHeader>
